...
微信公众号对HTML和CSS有一定的限制，我们尽可能地保证复制后的效果与预览一致，但可能会有细微差异。如果发现明显的样式问题，请提交issue。

### 如何插入图片？

目前支持通过Markdown语法插入网络图片，例如：`![图片描述](https://example.com/image.jpg)`。本地图片上传功能暂不支持。

### 支持哪些Markdown语法？

支持大部分常用的Markdown语法，包括标题、段落、列表、引用、代码块、表格等。详细语法请参考[Markdown基础语法](https://www.markdownguide.org/basic-syntax/)。
